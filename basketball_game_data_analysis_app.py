...
def correlation_significance_text(p_value: float) -> str:
    if pd.isna(p_value):
        return "样本不足，无法判断是否线性相关"
    if p_value < 0.01:
        return "在 1% 显著性水平下，可以认为该指标与胜负结果存在显著线性相关关系"
    if p_value < 0.05:
        return "在 5% 显著性水平下，可以认为该指标与胜负结果存在显著线性相关关系"
    return "未达到常用显著性水平，暂不能认为该指标与胜负结果存在显著线性相关关系"
...
def build_profile_scorecards(team_df: pd.DataFrame, league_df: pd.DataFrame) -> tuple[pd.DataFrame, str]:
    ...
    for category, metrics in profile_map.items():
        ...
        for metric, reverse in metrics:
            ...
            rows.append({"类别": category, "指标": METRIC_LABELS.get(metric, metric), "Z分数": relative_score})
...
def build_style_explanation(profile_df: pd.DataFrame, style_label: str) -> str:
    ...
    category_scores = profile_df.groupby("类别")["Z分数"].mean()
    offense_score = category_scores.get("进攻", np.nan)
    defense_score = category_scores.get("防守", np.nan)
        ...
        rows.append(
            {
                ...
                "相关系数 r": corr,
                "p 值": p_value,
                "解释": f"{correlation_label(corr)}；{correlation_significance_text(p_value)}",
            }
        )
...
<div class="overview-card">
    ...
    <div class="overview-card-text">将核心指标转换为 Z 分数后再比较，更方便判断球队是偏进攻、偏防守，还是更接近攻守均衡型<br>当前判定：<b>{style_label}</b></div>
    </div>
</div>
            """,
            unsafe_allow_html=True,
...
<div class="overview-card">
    ...
    <div class="overview-card-text">
        <b>计算公式：</b> Z分数 = (球队指标均值 - 联盟球队均值) / 联盟球队标准差<br>
...
<div class="module-card">
    ...
    if not corr_df.empty:
        ...
        st.markdown(
            """
<div class="module-card">
    <div class="module-card-title">胜负结果相关性排序</div>
